...
#        msft_h['Stock Splits'].mask(msft_h['Stock Splits'] == 0.0,1.0,inplace=True)
        ...
        Inv_1 = self.units*msft_h1.Close[0]*msft_h1['Stock Splits'][0]
        Inv_3 = []
        Inv_2 = []
        print(msft_h1.Close[0])
        for ii in range(1,msft_h1.Dividends.shape[0]):
                Inv_2 = (Inv_1 + (Inv_1*msft_h2_ch.Close[ii]))
                if(msft_h1['Stock Splits'][ii] > 2.0):
                    Inv_1 = msft_h1['Stock Splits'][ii]*(Inv_2)
                    print("INV_1: {} INV_2: {} Stock Splits {} Per_change {} Dividents {}".format(Inv_1,Inv_2, msft_h1['Stock Splits'][ii], msft_h2_ch.Close[ii], msft_h1.Dividends[ii] ))
                else:
                    Inv_1 = (Inv_2 + Inv_2*msft_h1.Dividends[ii]/100)
                    print("INV_1: {} INV_2: {} Stock Splits {} Per_change {} Dividents {}".format(Inv_1,Inv_2, msft_h1['Stock Splits'][ii], msft_h2_ch.Close[ii], msft_h1.Dividends[ii] ))
                    print("Market Closes at : ",msft_h1.Close[ii])
                Inv_3.append(Inv_1)
        
        Inv_3.insert(0, self.units* msft_h1.Close[0])
